...
import { clearAuthState, registerRequest } from "../features/auth/authAction";
...
function SignUp() {
    ...
    const { loading, message, error } = useSelector(state => state.auth.register);
            ...
            <div className="relative w-full max-w-md">
                ...
                <div className="relative bg-white/40 border border-zinc-800 rounded-2xl shadow-2xl p-8 backdrop-blur-xl">
                    <div className="flex flex-col items-center mb-8">
                        ...
                        <p className="text-zinc-600 text-sm mt-1.5">Create an account</p>
                    </div>

                    <form onSubmit={handleSubmit} className="space-y-5 text-sm">
                        <div>
                            <label className="block mb-1.5 text-zinc-300 font-medium">Name</label>
                            <Input icon={LuUser} placeholder="Enter your name" name="name" value={registerData.name} onChange={handleChange} />
                        </div>
                        <div>
                            <label className="block mb-1.5 text-zinc-800 font-medium">Email</label>
                            <Input icon={LuMail} placeholder="you@company.com" name="email" value={registerData.email} onChange={handleChange} />
                        </div>
                        <div>
                            <label className="block mb-1.5 text-zinc-300 font-medium">Password</label>
                        ...
                        > {loading ?
                            ...
                            :
                            <>Register < LuArrowRight className="size-4" /></>
                            }
                        </button>
                    </form>
